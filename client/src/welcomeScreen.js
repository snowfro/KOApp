...
class WelcomeScreen extends React.Component {
  ...
  handleAddressCreditSendClick(){
    ...
    if (this.state.purchaseCredit){
      const amountToSend = drizzleState.contracts.KOPrintRegistry.pricePerPrintIntlShipInWei['0x0'].value;
...
render(){
  ...
  const creditsToGive = this.props.drizzleState.contracts.KOPrintRegistryMinter.managerAddressToCreditsToGive[this.props.creditsToGiveKey];
  const owner1 = this.props.drizzleState.contracts.KOPrintRegistryMinter.owner1[this.props.owner1Key];
  const owner2 = this.props.drizzleState.contracts.KOPrintRegistryMinter.printerAddress[this.props.owner2Key];
  ...
  {this.state.purchaseCredit &&
    ...
    <h4>You can purchase print credits for an Ethereum address or an individual KO Edition for {pricePerPrintIntlShipInWei && (web3.utils.fromWei((pricePerPrintIntlShipInWei.value).toString(), 'ether'))}Ξ each.</h4>
    <ul>
    <li>Credits can only go towards a print. No "NFC Only" credits exist.</li>
    <li>You can assing a print credit to an Edition that has not yet sold following KO's token numbering standard. Contact us for more details.</li>
    ...
    <div className="input-group flex-nowrap">
    <div className="input-group-prepend">
    <span className="input-group-text" id="address-wrapping">Edition Number</span>
    </div>
    ...
    {creditsToGive.value>0? <h1>You have {creditsToGive.value} print {creditsToGive.value>1?'credits':'credit'}  to give!</h1>:<h1>Assign Print Credits to User or Edition</h1>}
    <h4>You can assign your credits to an address or a specific KO Edition.</h4>
    <br />
    <div className="input-group flex-nowrap">
    <div className="input-group-prepend">
    <span className="input-group-text" id="address-wrapping">Recipient Address</span>
    ...
    <div className="input-group flex-nowrap">
    <div className="input-group-prepend">
    <span className="input-group-text" id="address-wrapping">KO Edition</span>
    </div>
